...
impl Renderer {
    pub fn new(connection: &Connection, window: &Window) -> Result<Self> {
        ...
        // SAFETY: the wl_display lives as long as `Connection`, and the wl_surface lives
        // as long as the `Window` (which owns the `LayerSurface`). Both must outlive
        // this `Renderer`, which the caller upholds by dropping `Renderer` first.
    ...
    pub fn blit_to(&self, encoder: &mut wgpu::CommandEncoder, dst: &wgpu::TextureView) {
        ...
        self.surface
            .blitter
            .copy(device, encoder, &self.surface.target_view, dst);
    }
}
